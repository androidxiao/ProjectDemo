package cn.expand.layout.cn.widget;
...
public class ExpandableLayout extends LinearLayout {
    ...
    public interface Renderer<T, E> {
        void renderParent(View view, T model, boolean isExpanded, int parentPosition);

        void renderChild(View view, E model, int parentPosition, int childPosition);
    }
    ...
    public <T> void setExpandListener(ExpandCollapseListener.ExpandListener<T> expandListener) {
        this.expandListener = expandListener;
    }

    public <T> void setCollapseListener(ExpandCollapseListener.CollapseListener<T> collapseListener) {
    ...
    public <T, E> void addChild(T parent, E child) {
    ...
    public <T, E> void addChildren(T parent, List<E> children) {
        int parentIndex = NO_INDEX;
        for (int i = 0; i < sections.size(); i++) {
            if (sections.get(i).parent.equals(parent)) {
    ...
    private <E> void notifyItemAdded(int parentIndex, E child) {
        if (renderer == null) {
            return;
        }
    ...
    private <E> void notifyItemAdded(int parentIndex, List<E> children) {
        if (renderer == null) {
            return;
        }
        ViewGroup parentView = (ViewGroup) getChildAt(parentIndex);
    ...
    private <T> void expand(@NonNull T parent) {
        for (int i = 0; i < sections.size(); i++) {
    ...
    private <T> void collapse(@NonNull T parent) {
        for (int i = 0; i < sections.size(); i++) {
            if (parent.equals(sections.get(i).parent)) {
                ViewGroup sectionView = ((ViewGroup) getChildAt(i));
